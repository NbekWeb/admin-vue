...
import { register_API } from "@/service/login";
export default {
	setup() {
		return {
			v$: useVuelidate(),
	...
	// Form valid emas bo‘lsa, davom ettirmaymiz
	if (this.v$.$invalid) return;

	try {
		const [error, response] = await register_API({ ...this.user });

		if (error) {
			this.isRegisterError = true;
			this.regError = error.message || "Ro‘yxatdan o‘tishda xatolik yuz berdi.";
			return;
		}

		console.log("response", response);

		// Muvaffaqiyatli ro‘yxatdan o‘tdi, tokenni saqlaymiz
		...
		this.isRegisterError = true;
		this.regError = e.message || "Kutilmagan xatolik yuz berdi.";
	}
}
								...
								<div class="p-2 mt-4">
									...
									>
										...
										<div class="mb-3">
											...
											/>
											<div
												v-for="(item, index) in v$.user.phone
													.$errors"
												:key="index"
												class="invalid-feedback"
											>
										...
										<div class="mb-3">
											...
											/>
											<div
												v-for="(item, index) in v$.user.last_name
													.$errors"
												:key="index"
												class="invalid-feedback"
											>
										...
										<div class="mb-3">
											...
											<input
												...
												:class="{
													'is-invalid':
														submitted &&
														v$.user.first_name.$error,
												}"
												placeholder="Enter first name address"
											/>
											<div
												v-for="(item, index) in v$.user.first_name
													.$errors"
												:key="index"
												class="invalid-feedback"
											>
										...
										<!-- <div class="mb-3">
											...
											>
												<span
													v-if="v$.user.username.required.$message"
													>{{
										...
										<!-- <div class="mb-2">
											...
											>
												<span
													v-if="v$.user.password.required.$message"
													>{{
